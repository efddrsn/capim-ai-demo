...
const HomePage: React.FC = () => {
  ...
  // Combinar todas as sugestões e filtrar baseado no texto digitado
  const allSuggestions = [
    ...actionSuggestions.map(action => ({ text: action, type: 'action' as const })),
    ...questionSuggestions.map(question => ({ text: question, type: 'question' as const }))
  ];
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-8 max-w-4xl mx-auto relative">
        ...
        {/* Suggestion Pills - Horizontal Layout with Filtering */}
        {chatState === 'suggestions' && (
          <div className="relative z-10">
            {filteredSuggestions.length > 0 ? (
              ...
              <div className="text-center py-4 text-gray-500">
                ...
                <p className="text-sm">Nenhuma sugestão encontrada para "{chatMessage}"</p>
                <p className="text-xs mt-1">Tente outro termo ou pressione Enter para continuar</p>
              </div>
            )}
          </div>
        )}
